...
enum MenuBarAnimationRender {
    ...
    static func run(outPath: String) -> Bool {
        ...
        let timeline: [Step] = [
            ...
            Step(label: "hint ⌘L",   apply: { view.flashKeyHint(chord: "⌘L", action: "Focus →", now: vt) }, holdFrames: 18),
            Step(label: "focus->4",  apply: { view.apply(state: state([1,2,3,4], focus: 3), managing: true, now: vt) }, holdFrames: 18),
            Step(label: "hint ⌘2",   apply: { view.flashKeyHint(chord: "⌘2", action: "Width 50%", now: vt) }, holdFrames: 18),
